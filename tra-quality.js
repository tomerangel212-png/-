...
(() => {
  ...
  style.textContent = `
    ...
    .tra-skip{position:fixed;z-index:2147483647;top:8px;right:8px;transform:translateY(-150%);background:#fff;color:#111;padding:10px 14px;border-radius:10px;font:700 14px/1.2 system-ui;text-decoration:none;box-shadow:0 6px 24px #0004}
    .tra-skip:focus{transform:none}
    .tra-quality-badge{position:fixed;z-index:2147483646;left:max(10px,env(safe-area-inset-left));bottom:max(10px,env(safe-area-inset-bottom));display:inline-flex;gap:6px;align-items:center;padding:8px 11px;border:1px solid #ffffff2e;border-radius:999px;background:#0d1715e8;color:#fff;font:800 12px/1 system-ui;box-shadow:0 8px 26px #0005;backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);text-decoration:none}
    .tra-quality-badge strong{color:#f4cb67}
    .tra-network-status{position:fixed;z-index:2147483645;left:max(10px,env(safe-area-inset-left));bottom:max(54px,calc(env(safe-area-inset-bottom) + 54px));max-width:min(340px,calc(100vw - 20px));padding:9px 12px;border-radius:12px;background:#151b20e8;color:#fff;font:700 12px/1.4 system-ui;box-shadow:0 8px 24px #0004;opacity:0;transform:translateY(8px);pointer-events:none;transition:.18s ease}
    .tra-network-status.show{opacity:1;transform:none}
    @media(prefers-reduced-motion:reduce){*,*::before,*::after{scroll-behavior:auto!important;animation-duration:.001ms!important;animation-iteration-count:1!important;transition-duration:.001ms!important}}
    @media(max-width:520px){.tra-quality-badge{font-size:11px;padding:7px 9px}}
  `;
  ...
  const track = (event, properties = {}) => {
    try { window.posthog?.capture?.(event, { page, tra_version: "9.9", tra_quality: "10/10", ...properties }); } catch {}
  };
  ...
  const mount = () => {
    ...
    if (!document.querySelector(".tra-quality-badge")) {
      ...
      badge.setAttribute("aria-label", "TRA גרסה 9.9, יעד איכות 10 מתוך 10");
      badge.innerHTML = `<span>TRA 9.9</span><strong>10/10</strong>`;
      document.body.append(badge);
    }

    track("tra_page_quality_loaded", { online: navigator.onLine });
  ...
  if ("serviceWorker" in navigator && location.protocol === "https:") {
    ...
    navigator.serviceWorker.register(sw.href, { scope: rootUrl.pathname }).catch(() => {});
  }
  ...
  fetch(new URL("TRA_VERSION.json", rootUrl), { cache: "no-store" })
    ...
    .then(data => {
      ...
      const badge = document.querySelector(".tra-quality-badge span");
      if (badge) badge.textContent = `TRA ${data.version}`;
    })
    .catch(() => {});
