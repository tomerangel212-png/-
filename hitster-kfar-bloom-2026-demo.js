...
async function startPreview(){if(!state.current?.preview)return false;const audio=$("audio");if(!audio.src)prepareAudio(state.current.preview);clearTimeout(state.previewTimer);try{audio.currentTime=0;}catch{}try{await audio.play();$("play").textContent="■ עצירה";state.previewTimer=setTimeout(stopAudio,PREVIEW_SECONDS*1000);$("status").textContent=`▶ מנגן אוטומטית ${PREVIEW_SECONDS} שניות`;return true;}catch(error){$("play").textContent=`▶ ${PREVIEW_SECONDS} שניות`;$("play").disabled=false;if(error?.name === "NotAllowedError"){$("status").textContent="Safari חסם את ההפעלה האוטומטית לאחר החיפוש. לחצו פעם אחת על כפתור ההשמעה כדי לנגן 30 שניות.";}else{$("status").textContent="האודיו לא הצליח להתנגן. נסו שוב או עברו לקלף הבא.";}return false;}}
