...
const claimDrawButton = document.querySelector("#claim-draw");
...
function repetitionCount(game=chess){
...
function halfMoveClock(game=chess){return Number(game.fen().split(" ")[4])||0;}
function claimableDraw(game=chess){
...
function renderClock(){const over=resultState().over;["w","b"].forEach(color=>{const el=color==="w"?whiteClock:blackClock;el.querySelector("strong").textContent=format(seconds[color]);el.classList.toggle("active",!over&&chess.turn()===color);});}
function resultState(game=chess){
  ...
  return {over:false,reason:null,winner:null,loser:null,text:null,claimable:claim};
}
function status(){
  ...
  if(result.claimable)return `תור ${turn}. אפשר לדרוש תיקו — ${result.claimable.text}.`;
  return chess.isCheck()?`שח על ${turn}! מותר לבצע רק מהלך חוקי שמוציא את המלך משח.`:`תור ${turn}.`;
}
function legalMoves(square=selected,game=chess){return square?game.moves({square,verbose:true}):[];}
function strictLegalMove(from,to,promotion=null){
  const candidates=legalMoves(from).filter(move=>move.to===to);
  if(!candidates.length)return null;
  if(candidates.some(move=>move.promotion))return candidates.find(move=>move.promotion===promotion)||null;
  return promotion===null?candidates[0]:null;
}
function commitStrictLegalMove(from,to,promotion=null){
  const move=strictLegalMove(from,to,promotion);
  if(!move)return null;
  return chess.move(move.promotion?{from:move.from,to:move.to,promotion:move.promotion}:{from:move.from,to:move.to});
}
function render(){
  const result=resultState();
  ...
  for(const sq of squareNames()){
    ...
    button.disabled=result.over || botThinking || !isHumanTurn();
  ...
  undoButton.disabled=chess.history().length===0 || botThinking || result.over;
  if(claimDrawButton)claimDrawButton.disabled=!result.claimable || botThinking || !isHumanTurn();
  renderClock();
}
function promotionChoiceRequired(piece,to){
  if(piece?.type!=="p"||!to)return false;
  return (piece.color==="w"&&to[1]==="8")||(piece.color==="b"&&to[1]==="1");
}
function promote(from,to){
  const piece=chess.get(from);
  if(!promotionChoiceRequired(piece,to))return {cancelled:false,promotion:null};
  const raw=window.prompt("קידום רגלי: בחרו מלכה (Q), צריח (R), רץ (B) או סוס (N).", "Q");
  if(raw===null)return {cancelled:true,promotion:null};
  const value=String(raw).trim().toLocaleLowerCase("he");
  const aliases={q:"q",מלכה:"q",r:"r",צריח:"r",b:"b",רץ:"b",n:"n",סוס:"n"};
  return {cancelled:false,promotion:aliases[value]||"q"};
}
...
function blockIllegalMove(from,to,error){track("chess_illegal_move_blocked",{from,to,...(error?{error:String(error)}:{})});selected=null;render();statusEl.textContent="מהלך לא חוקי נחסם — המלך אינו יכול להיכנס לשח, להישאר בשח או להכות כלי שמוגן בידי היריב.";}
function claimDraw(){
  const result=resultState();
...
function press(square){
  if(resultState().over||botThinking||!isHumanTurn()){selected=null;render();return;}
  const piece=chess.get(square);
  if(selected){
    const candidateMoves=legalMoves(selected).filter(move=>move.to===square);
    if(candidateMoves.length){
      const promotion=promote(selected,square);
      if(promotion.cancelled){selected=null;render();statusEl.textContent="קידום הרגלי בוטל.";return;}
      try{
        const done=commitStrictLegalMove(selected,square,promotion.promotion);
        selected=null;
        if(done){
          started=true;
          ...
          finishGameIfNeeded();
          render();
          if(!resultState().over)scheduleBotMove();
          return;
        }
      }catch(err){
        blockIllegalMove(selected,square,err);
        return;
      }
    }
    blockIllegalMove(selected,square);
    return;
  }
...
function scheduleBotMove(){
  const result=resultState();
  if(!isBotMode()||chess.turn()!=="b"||result.over)return;
  if(result.claimable){claimDraw();return;}
  ...
  botTimer=setTimeout(()=>{
    ...
    const done=commitStrictLegalMove(move.from,move.to,move.promotion||null);
    botThinking=false;
    botTimer=null;
    if(done){
      started=true;
...
function reset(){
  chess=new Chess();selected=null;seconds={w:300,b:300};started=false;botThinking=false;claimedDrawReason=null;
...
function regressionCheck(){
  ...
  const addGame=(name,moves,assertion)=>{const t=new Chess();for(const [from,to,promotion] of moves)t.move(promotion?{from,to,promotion}:{from,to});tests.push({name,pass:Boolean(assertion(t))});};
  ...
  addState("promotion chooser appears only at the final pawn rank","k7/4P3/8/8/8/8/4p3/4K3 w - - 0 1",t=>promotionChoiceRequired(t.get("e7"),"e8")&&!promotionChoiceRequired(t.get("e7"),"e6")&&promotionChoiceRequired(t.get("e2"),"e1")&&!promotionChoiceRequired(t.get("e2"),"e3"));
  ...
  addState("fifty move rule is claimable","r3k3/8/8/8/8/8/8/4K2R w - - 100 51",t=>{const result=resultState(t);return !result.over&&result.claimable?.reason==="fifty_move";});
  addState("seventy five move rule is automatic","r3k3/8/8/8/8/8/8/4K2R w - - 150 76",t=>{const result=resultState(t);return result.over&&result.reason==="seventy_five_move";});
  const repetitionCycle=[["g1","f3"],["g8","f6"],["f3","g1"],["f6","g8"]];
  addGame("threefold repetition is claimable",[...repetitionCycle,...repetitionCycle],t=>{const result=resultState(t);return !result.over&&result.claimable?.reason==="threefold_repetition";});
  addGame("fivefold repetition is automatic",[...repetitionCycle,...repetitionCycle,...repetitionCycle,...repetitionCycle],t=>{const result=resultState(t);return result.over&&result.reason==="fivefold_repetition";});
