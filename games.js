...
function status(){if(chess.isCheckmate()) return `מט! ${chess.turn()==="w"?"השחור":"הלבן"} ניצח.`;if(chess.isStalemate()||chess.isThreefoldRepetition()||chess.isInsufficientMaterial()||chess.isDraw()) return "תיקו. משחק חדש?";const turn=chess.turn()==="w"?"לבן":"שחור";return chess.isCheck()?`שח על ${turn}! מצאו מהלך הצלה.`:`תור ${turn}.`;}
function legalMoves(){return selected?chess.moves({square:selected,verbose:true}):[];}
...
function press(square){const piece=chess.get(square);if(selected){const move=legalMoves().find(m=>m.to===square);if(move){const done=chess.move(promote({from:selected,to:square,promotion:move.promotion}));selected=null;if(done){started=true;track("chess_move_played",{color:done.color,piece:done.piece,from:done.from,to:done.to,notation:done.san});render();return;}}selected=null;}if(piece?.color===chess.turn())selected=square;render();}
function reset(){chess=new Chess();selected=null;seconds={w:300,b:300};started=false;clearInterval(interval);interval=null;track("chess_game_started",{mode:"local_5_min"});render();}
