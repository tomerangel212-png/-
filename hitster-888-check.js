...
check("played cards are globally unique", engine.includes("state.used.push(card.id)") && engine.includes("function unusedCards") && engine.includes("!used[card.id]"));
...
check("engine prepares a verified preview before a card can be drawn", engine.includes("function primeNextCard") && engine.includes("function resolvePlayablePreview") && engine.includes("function loadPreviewIntoPlayer") && engine.includes("var nextReady") && engine.includes("if (!ready)"));
...
check("audio retries the same card after a source failure", engine.includes("function recoverCurrentPreview") && engine.includes("prepareCurrentPreview(card, { force: true })") && engine.includes("function forgetPreview"));
check("audio enforces every 30-second cutoff", engine.includes("PREVIEW_SECONDS = 30") && engine.includes("PREVIEW_SECONDS * 1000") && engine.includes("audio.currentTime >= PREVIEW_SECONDS"));
...
check("mobile entry preserves Kfar Blum start flow", mobile.includes('id="game"') && mobile.includes('allow="autoplay"') && mobile.includes('"hitster-888.html"') && mobile.includes('"hitster-888-en.html"') && mobile.includes('params.get("entry")'));
check("mobile wrapper does not bypass continue/reset", !mobile.includes("next.click()") && !mobile.includes("play.click()"));
