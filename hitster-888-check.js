...
if (!kfarRoute.includes('location.replace("hitster-888.html?entry=kfar-bloom")') || !kfarRoute.includes("HITSTER 888")) failures.push("Kfar Blum route is not unified with the canonical 888 deck");
if (!legacyRoute.includes('location.replace("hitster-888-en.html?entry=international")') || !legacyRoute.includes("888 cards, exactly")) failures.push("legacy international route is not unified with the English 888 interface");
if (!english.includes('<html lang="en" dir="ltr">') || !english.includes("888 verified Hebrew A-list song cards") || !english.includes('src="hitster-kfar-bloom-2026-demo.js')) failures.push("English HITSTER interface is not wired to the verified 888 deck");
...
if (hub.includes("350 הקלפים") || hub.includes("444 קלפי") || hub.includes("57 כרטיס") || hub.includes("1,000 כרטיס")) failures.push("HITSTER hub contains a stale non-888 song count");
...
if (!sw.includes("hitster-hebrew-alist-888.json") || !sw.includes("hitster-888.html") || !sw.includes("hitster-888-en.html") || !sw.includes("v24-hitster-888-all-routes")) failures.push("all 888 routes are not cached with an 888-versioned offline cache");
